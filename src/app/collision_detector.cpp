...
CollectionResult TryCollectPoint(geom::Point2D a, geom::Point2D b,
    ...
    // пскольку при сборе заказов придётся учитывать перемещение даже на
    ...
    for (size_t i = 0; i < provider.GatherersCount(); ++i) {
        ...
        for (size_t j = 0; j < provider.ItemsCount(); ++j) {
            ...
            if (collect_result.IsCollected(gatherer.width + item.width)) {
                detected_events.emplace_back(j, i, collect_result.sq_distance,
                                             collect_result.proj_ratio);
            }
        }
    }

    std::ranges::sort(detected_events,
